...
import { GET as listAnalyses } from "../app/api/v1/runs/[runId]/analyses/route";
...
describe("AgentScope run APIs without persistent storage", () => {
  ...
  it("returns an explicit configuration response for derived analyses", async () => {
    vi.stubEnv("DATABASE_URL", "");

    const response = await listAnalyses(
      new Request("http://localhost/api/v1/runs/run_1/analyses"),
      { params: Promise.resolve({ runId: "run_1" }) },
    );
